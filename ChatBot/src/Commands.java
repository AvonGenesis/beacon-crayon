...
public class Commands {
    public static void checkCommands(String msg, String user){
        ...
        Jedis jedis = new Jedis("redis://:beacon@lotus.snax.io:6379/0");
        if(cmdMsg[0].contains("queue") || cmdMsg[0].contains("leave") || cmdMsg[0].contains("gold") || cmdMsg[0].contains("attack")){
            toSend = cmdMsg[0] + ":" + user;
            long value = jedis.publish("worker", toSend);
        } else if (cmdMsg[0].contains("role") || cmdMsg[0].contains("move") || cmdMsg[0].contains("buy")){
